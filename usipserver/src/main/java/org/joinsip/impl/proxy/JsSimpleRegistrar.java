...
public class JsSimpleRegistrar {
    ...
    public void register(JsAddress toUri, JsAddress contactUri, int expires) {
        ...
        Log.e("SREG", regTable.toString());
    }

    // TODO: 过滤
    private void checkRepeat(JsAddress paramJsAddress)
    {
        // 过滤相同sip
        if (this.regTable.containsKey(paramJsAddress.getUser()))
        {
            this.regTable.remove(paramJsAddress.getUser());
            return;
        }

        // 过滤相同ip
        Iterator localIterator = this.regTable.keySet().iterator();
        while (localIterator.hasNext())
        {
            String key = (String)localIterator.next();
            if (TextUtils.equals(this.regTable.get(key).getContactUri().getHost(), paramJsAddress.getHost())) {
                this.regTable.remove(key);
            }
        }
    }
